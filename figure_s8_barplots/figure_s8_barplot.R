...
sums_all_rank <- read.csv("/Users/ashfrana/Desktop/code/abcd_sex_pfn_replication/figure_s8_barplots/grouped_network_tables/gams_replication_grouped_networks.csv")
...
ggplot(sums_all_col_rank, aes(x = reorder(network, +vertecies), y = vertecies, fill=network, alpha=factor(sex)))+
  ...
  theme(legend.position="none") + theme(axis.text.x = element_text(size= 11), axis.text.y = element_text(size= 11)) + scale_x_discrete(label=labelmap$netLabel)
...
ggplot(sums_all_col_rank, aes(x = reorder(network, +weights), y = weights, fill=network, alpha=factor(sex)))+
