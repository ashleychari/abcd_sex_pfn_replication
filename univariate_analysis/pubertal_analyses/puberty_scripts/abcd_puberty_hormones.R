        ...
        for (j in 1:FeaturesQuantity_NonZero){
            ...
            if (both_sexes == "FALSE"){
                ...
                if (include_age == "TRUE" & hormone == "ert"){
                    gam_analysis <- bam(loading_data ~  hormone_scr_ert_mean + oSex + Motion + s(AgeYears, k=4) + s(abcd_site, bs='re', k=4), method = "fREML", data = behavior_new);
                    lm_analysis <- lmer(loading_data ~  hormone_scr_ert_mean + oSex + Motion + AgeYears + (1|abcd_site), data = behavior_new);
                }else if (include_age == "FALSE" & hormone == "ert"){
                    gam_analysis <- bam(loading_data ~  hormone_scr_ert_mean + oSex + Motion + s(abcd_site, bs='re', k=4), method = "fREML", data = behavior_new);
                    lm_analysis <- lmer(loading_data ~  hormone_scr_ert_mean + oSex + Motion + (1|abcd_site), data = behavior_new);
                }else if (include_age == "TRUE" & hormone == "dhea"){
                    gam_analysis <- bam(loading_data ~ hormone_scr_dhea_mean + oSex + Motion + s(AgeYears, k=4) +  s(abcd_site, bs='re', k=4), method = "fREML", data = behavior_new);
                    lm_analysis <- lmer(loading_data ~ hormone_scr_dhea_mean  + oSex + Motion + AgeYears + (1|abcd_site), data = behavior_new);
                }else{
                    gam_analysis <- bam(loading_data ~ hormone_scr_dhea_mean + oSex + Motion +  s(abcd_site, bs='re', k=4), method = "fREML", data = behavior_new);
                    lm_analysis <- lmer(loading_data ~ hormone_scr_dhea_mean  + oSex + Motion + (1|abcd_site), data = behavior_new);
                }
            ...
            hormone_T <- summary(lm_analysis)$coefficients[2, 3];
            if (hormone_T < 0) {
