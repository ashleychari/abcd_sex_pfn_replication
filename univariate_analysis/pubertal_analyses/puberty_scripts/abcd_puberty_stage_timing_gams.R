...
create_behavior_df <- function(filepath){
  ...
  behavior_new$pds_p_category <- as.factor(behavior_df$pds_p_category)
  return (behavior_new)
}
...
main <- function(behavior_filename, set, test_name, include_age){
    ...
    behavior_new <- create_behavior_df(behavior_filename)
    ...
    for (i in 1:17){
        ...
        for (j in 1:FeaturesQuantity_NonZero){
            ...
            if (include_age == "TRUE"){
                gam_analysis <- bam(loading_data ~  pds_p_category + oSex + Motion + 
                s(AgeYears, k=4) + s(abcd_site, bs='re', k=4), method = "fREML", data = behavior_new);
                lm_analysis <- lmer(loading_data ~ pds_p_category + oSex + Motion +
                AgeYears + (1|abcd_site), data = behavior_new);
            }else{
                gam_analysis <- bam(loading_data ~ pds_p_category + oSex + Motion +
                 s(abcd_site, bs='re', k=4), method = "fREML", data = behavior_new);
                lm_analysis <- lmer(loading_data ~  pds_p_category + oSex + Motion +
                (1|abcd_site), data = behavior_new);
            }
...
print("IN abcd_puberty_stage_timing_gams.R")
print(args)
set <- args[1]
...
# Run script
main(behavior_file, set, test_name, include_age) # discovery sample
