...
if __name__ == "__main__":
    ...
    data_type = input_filename.split(".")[1]
    if data_type == 'csv':
        data_to_be_converted = pd.read_csv(input_filename)
        print(f"Value from file to be converted: {data_to_be_converted['0'].values[0]}")
    else:
        data_to_be_converted = np.load(input_filename)
        print(f"Value from file to be converted: {data_to_be_converted[0]}")


    data_array = np.zeros((1, 59412))
    data_array[0,:] = data_to_be_converted
    ...
    new_cifti_img = nib.Cifti2Image(np.asanyarray(data_array), header=cifti_header)
