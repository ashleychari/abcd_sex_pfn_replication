...
def create_subject_id_tables(subject_ids, dataset_label):
    ...
    if len(subject_ids_fold1) != len(subject_ids_fold2):
        # check which list is less than and by how much
        list1_lt = len(subject_ids_fold1) < len(subject_ids_fold2)
        list2_lt = len(subject_ids_fold2) < len(subject_ids_fold1)

        if list1_lt:
            diff = len(subject_ids_fold2) - len(subject_ids_fold1)
            for i in range(diff):
                subject_ids_fold1.append("NA")

        if list2_lt:
            diff = len(subject_ids_fold1) - len(subject_ids_fold2)
            for i in range(diff):
                subject_ids_fold2.append("NA")


    subject_id_table_discovery = pd.DataFrame()
    subject_id_table_discovery['fold1_ids'] = subject_ids_fold1
    subject_id_table_discovery['fold2_ids'] = subject_ids_fold2
    subject_id_table_discovery.to_csv(f"/cbica/projects/ash_pfn_sex_diff_abcd/results/multivariate_analysis/network_specific_models_results/network_{network_num}/{dataset_label}_subject_ids_2_folds.csv")

def create_coef_table(coefs, dataset_label):
    coefs_table = pd.DataFrame()
    i = 1
    for coef_fold in coefs:
        coefs_table[f"fold_{i}_coefs"] = coef_fold
        i += 1
    coefs_table.to_csv(f"/cbica/projects/ash_pfn_sex_diff_abcd/results/multivariate_analysis/network_specific_models_results/network_{network_num}/2foldcv_{dataset_label}_coefs.csv")
...
if __name__ == "__main__":
    ...
    discovery_matrix_filename = f"{matrices_folder}/discovery_network_matrix_nonzero_{network_num}.npy"
    replication_matrix_filename = f"{matrices_folder}/replication_network_matrix_nonzero_{network_num}.npy"

    accuracy_discovery, auc_discovery, specificity_discovery, decision_values_discovery, subject_ids_discovery, coefs_discovery, random_state_seed_discovery = run_network_specific_svm(discovery_matrix_filename, discovery_data_for_ridge, c_values)
    accuracy_replication, auc_replication, specificity_replication, decision_values_replication, subject_ids_replication, coefs_replication, random_state_seed_replication = run_network_specific_svm(replication_matrix_filename, replication_data_for_ridge, c_values)

    if os.path.isdir(f"/cbica/projects/ash_pfn_sex_diff_abcd/results/multivariate_analysis/network_specific_models_results/network_{network_num}") == False:
        os.mkdir(f"/cbica/projects/ash_pfn_sex_diff_abcd/results/multivariate_analysis/network_specific_models_results/network_{network_num}")
    ...
    metrics_table.to_csv(f"/cbica/projects/ash_pfn_sex_diff_abcd/results/multivariate_analysis/network_specific_models_results/network_{network_num}/2foldcv_network_{network_num}_metrics_table.csv")


    # Save decision values for discovery set
    np.save(f"/cbica/projects/ash_pfn_sex_diff_abcd/results/multivariate_analysis/network_specific_models_results/network_{network_num}/discovery_decision_values_2_folds", np.array(decision_values_discovery))

    # Save decision values for replication set
    np.save(f"/cbica/projects/ash_pfn_sex_diff_abcd/results/multivariate_analysis/network_specific_models_results/network_{network_num}/replication_decision_values_2_folds", np.array(decision_values_replication))

    # Save subject ids for both folds for discovery and replication
    create_subject_id_tables(subject_ids_discovery, "discovery")
    create_subject_id_tables(subject_ids_replication, "replication")

    # Create coefficient tables
    create_coef_table(coefs_discovery, "discovery")
    create_coef_table(coefs_replication, "replication")
